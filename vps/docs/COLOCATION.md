...
/root/<your-project>/          # clone your git repo here (sibling to rose-astrology)
  docker-compose.yml
  docker-compose.vps.yml       # localhost bind + .env.production
  .env.production              # gitignored, mode 600
  vps/                         # copy shared kit from fandango_watcher (or submodule)
    projects/<name>.env        # ports, compose files, secret mappings (no secrets)
    scripts/preflight.sh       # neighbor safety
    run_vps_cmd.py             # SSH from Windows
...
Use the shared **`vps/run_vps_cmd.py`** (Paramiko; reads `vps/host.env` + `vps/projects/<name>.env`):

```bash
cp vps/host.env.example vps/host.env   # set ROSE_SECRETS_VPS_MD path
python vps/run_vps_cmd.py --project my-app "docker ps"
python vps/run_vps_cmd.py --project my-app --sync-secrets
```

Password: `VPS_SSH_PASSWORD`, repo `secrets.vps.md`, Rose `secrets.vps.md`, or prompt.

Rose monorepo still has its own `run_vps_cmd.py` for Rose-only ops.
...
**Full detail:** see `vps/docs/COLOCATION.md` in any repo that copied the shared kit.

---

## 14. Deployed projects (examples)

| Project | Config | Path on VPS | Port |
|---------|--------|-------------|------|
| fandango_watcher | `vps/projects/fandango-watcher.env` | `/root/fandango-watcher` | `127.0.0.1:8787` |
| Rose Astrology | Rose monorepo | `/root/rose-astrology` | `0.0.0.0:7166` |

Add a row + `vps/projects/<name>.env` for each new co-located app.
