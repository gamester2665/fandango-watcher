...
### Release schemas: overview vs format targets

Fandango uses two different surfaces:

| Target kind | Example | What “disclosed” means |
|-------------|---------|-------------------------|
...
Deploy the Docker/VPS stack by building the Docker image (or installing with `uv` on the host), copying `config.yaml` and a populated `.env`, and running `watch` (or `docker compose` as in the repo). Verify the process with `GET http://127.0.0.1:8787/healthz` (or your bound host/port) and optional `GET /metrics` for heartbeat counters. The read-only dashboard on `/` exposes `/api/status`, `/api/purchases` (tail of `state/purchases.jsonl`), and `/api/revision` for live reload.
